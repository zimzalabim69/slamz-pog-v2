...
export interface GameStore {
  ...
  swarmActive: boolean;
  swarmCount: number;
  triggerWraithSwarm: (count?: number) => void;
  clearSwarm: () => void;
  shatteringIds: string[];
  markForShatter: (id: string) => void;
  finalizeShatter: (id: string) => void;
...
export const useGameStore = create<GameStore>()(
  subscribeWithSelector((set, get) => {
    ...
    return {
      ...
      swarmActive: false,
      swarmCount: 0,
      triggerWraithSwarm: (count = 5) => set({ swarmActive: true, swarmCount: count }),
      clearSwarm: () => set({ swarmActive: false, swarmCount: 0 }),
      shatteringIds: [],
  ...
  unlockedSlammers: ['standard'],
      ...
      setGameState: (gameState) => set((state) => {
        // Decrement throws when entering SLAMMED state
        let newThrows = state.throwsRemaining;
        let shouldSwarm = false;

        if (gameState === 'SLAMMED' && state.gameState !== 'SLAMMED') {
          newThrows = Math.max(0, state.throwsRemaining - 1);
          // If this was the final shot, trigger the swarm protocol
          if (newThrows === 0) shouldSwarm = true;
        }

        const nextState = { gameState, throwsRemaining: newThrows };
        
        // Use a slight delay to trigger the action so it doesn't collide with the state update
        if (shouldSwarm) {
          setTimeout(() => get().triggerWraithSwarm(12), 100);
        }
