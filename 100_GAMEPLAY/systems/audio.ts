...
/**
 * Shatter Sound: Digital Dial-up Horror
 * High-pitched screeching FM synthesis mixed with glitchy noise
 */
export function playShatterSound() {
  const ctx = getCtx();
  const now = ctx.currentTime;
  
  // 1. Screeching Carrier Tone (FM)
  ...
  carrier.frequency.exponentialRampToValueAtTime(12000, now + 0.1);
  carrier.frequency.exponentialRampToValueAtTime(50, now + 0.4);
  
  modulator.frequency.setValueAtTime(20, now);
  modulator.frequency.exponentialRampToValueAtTime(800, now + 0.2);
  
  modGain.gain.setValueAtTime(500, now);
  modGain.gain.exponentialRampToValueAtTime(2000, now + 0.1);
  ...
  for (let i = 0; i < bufferSize; i++) {
    // Bit-crushed noise
    const noise = Math.random() * 2 - 1;
    data[i] = Math.sign(Math.sin(i * 0.01) * noise) * (1 - i / bufferSize);
  }
  const source = ctx.createBufferSource();
  source.buffer = buffer;
  const filter = ctx.createBiquadFilter();
